...
export default {
  ...
  methods: {
    ...
    getJizuPoint (value) {
      console.log("这里中文地址", value)
      return new Promise((resolve, reject) => {
        var myGeo = new BMap.Geocoder();
        var pt = new BMap.Point(); //104.065735, 30.659462   四川省成都市青羊区文庙后街76号
    ...
    storeMap (BMap, map, dataInfo) {
      ...
      for (var i = 0; i < dataInfo.length; i++) {
        ...
        console.log("dataInfo[i].data.azdd", dataInfo[i].data.azdd)
        console.log("dataInfo[i].jizu", dataInfo[i].jizu)
        var marker = new BMap.Marker(new BMap.Point(dataInfo[i].jizu.lng, dataInfo[i].jizu.lat), {
          icon: icon1,
          setTop: true,
        ...
        // 点击图标弹窗
        this.openClick(BMap, map, marker, dataInfo[i].jizu.lat, dataInfo[i].jizu.lng, dataInfo[i].data)
        /** 开始绘制线段 *****************************/
        ...
        let obj = {
          ...
          to: {
            lng: dataInfo[i].jizu.lng, 
            lat: dataInfo[i].jizu.lat
          }
        }
        array.push(obj)
    ...
    async getData () {
      this.axios({
        ...
        zgdt.forEach(item =>{
          ...
          let pt = new BMap.Point(); //104.065735, 30.659462   四川省成都市青羊区文庙后街76号
          myGeo.getPoint(item.data.azdd + item.data.azxxdz, (point) => {
              item.jizu.lat = point.lat
              item.jizu.lng = point.lng
          })
        })
