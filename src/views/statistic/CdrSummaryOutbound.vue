    ...
    <a-card class="table-search" :bordered="false">
      <a-form :layout="advanced ? 'vertical' : 'inline'" :class="advanced ? 'advanced' : 'normal'">
        ...
        <a-row :gutter="16">
          ...
          <a-col v-bind="colLayout">
            <a-form-item label="呼叫号码">
              <a-input v-model="queryParam.dst" />
            </a-form-item>
          </a-col>
          <a-col v-bind="colLayout">
...
export default {
  ...
  methods: {
    loadDataTable (parameter) {
      return this.axios({
        url: '/statistic/CdrSummary/outbound',
        params: Object.assign(parameter, this.queryParam)
      }).then(res => {
        this.calldate = [res.result.begin_day ? this.moment(res.result.begin_day, 'YYYY-MM-DD') : '', res.result.end_day ? this.moment(res.result.end_day, 'YYYY-MM-DD') : '']
    ...
    handleExport () {
      // let where = '1 = 1 and calltype="out"'
      // if (this.queryParam.dst) {
      //   where += ' and dst = "' + this.queryParam.dst + '"'
      // }
      ...
      //   title: '您确认导出所有数据吗?',
      //   onOk () {
      //     window.open(`${process.env.VUE_APP_API_BASE_URL}statistic/CdrSummary/outboundExport?where=` + encodeURIComponent(where))
      //   }
      // })
