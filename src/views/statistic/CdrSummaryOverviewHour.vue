  ...
  <a-card>
    <a-form class="search" layout="inline">
      <a-form-item label="呼叫时间">
        <a-date-picker @change="getSearchDate" v-model="date"></a-date-picker>
      </a-form-item>
...
export default {
  ...
  methods: {
    loadDataTable (parameter) {
      return this.axios({
        url: '/statistic/CdrSummary/overviewHour',
    ...
    handleExport () {
      ...
      //   where += ' and calldate >= "' + this.queryParam.starttime + sTime + '" and calldate <= "' + this.queryParam.endtime + eTime + '"'
      // } else {
      //   where = ''
      // }
      // this.$confirm({
      //   title: '您确认导出所有数据吗?',
      //   onOk () {
      //     window.open(`${process.env.VUE_APP_API_BASE_URL}statistic/CdrSummary/overviewExport?where=` + encodeURIComponent(where) + '&action=overview_hour')
      //   }
      // })
