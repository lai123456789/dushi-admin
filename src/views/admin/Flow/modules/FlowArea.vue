  ...
  <div style="width: 100%; height: 100%; overflow: hidden; position: relative;">
    ...
    <div
      ...
      <flow-node
        ...
        @ok="(val) => { getCurrentSelect(index,val) }"
  ...
  methods: {
    ...
    quote () {
      ...
      that.clipboard.forEach(function (node, index) {
        ...
        newNode.quoteId = node.quoteId || node.id
    ...
    deleteNode () {
      const that = this
      // 根据当前选中的节点查找与其相连的所有连线
      const conns = that.getConnectionsByNodeId(that.currentSelect.id)
      // 删除指定的连线
      conns.forEach(conn => {
      ...
      // 删除linkList中的连线
      conns.forEach(conn => {
        that.flowData.linkList = that.flowData.linkList.filter(item => !(item.sourceId === conn.sourceId && item.targetId === conn.targetId))
      })
      const nodeList = that.flowData.nodeList
      // 查找当前选中的节点
      const inx = nodeList.findIndex(node => node.id === that.currentSelect.id)
      // 删除当前选中的节点
    ...
    addNewNode (node) {
      ...
      if (newNode.type === 'start' || newNode.type === 'end' ||
          newNode.type === 'event' || newNode.type === 'gateway1' ||
          newNode.type === 'gateway2' || newNode.type === 'gateway3' ||
          newNode.type === 'gateway4' || newNode.type === 'gateway5') {
        newNode.x = x - 25
        newNode.width = 50
      } else {
        newNode.x = x - 60
    ...
    getCurrentSelect (index, currentSelect) {
      this.$emit('ok', index, currentSelect)
    },
    isMultiple (callback, currentSelect) {
