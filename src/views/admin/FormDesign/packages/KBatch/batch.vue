    ...
    >
      ...
      >
        <KFormModelItem
          ...
          v-model="record1[item.model]"
          @input="handleInput"
        />
      </template>
      <template slot="dynamic-delete-button" slot-scope="text, record2">
        <a-icon
          v-if="!disabled"
          class="dynamic-delete-button"
          type="minus-circle-o"
          @click="removeDomain(record2)"
        />
      </template>
    </a-table>
...
export default {
  ...
  components: {
    KFormModelItem: () => import('./module/KFormModelItem')
  },
  watch: {
    value: {
