    ...
    <a-card :bordered="false">
      ...
      >
        ...
        <div slot="action" slot-scope="text, record">
          <a @click="handleList(record)" :disabled="record.type!=='basic'">设计</a>
  ...
  methods: {
    ...
    handleEdit (record) {
      this.$refs.dictForm.show({
        ...
        url: '/admin/dict/edit?category=1',
        record: record,
    ...
    handleSort (record) {
      this.$refs.dictSort.show({
        action: 'sort',
        title: '排序',
        url: '/admin/dict/sort?category=1',
        parent_number: this.queryParam.parent_number || record.number || ''
      })
    },
    // 设计
    handleList (record) {
