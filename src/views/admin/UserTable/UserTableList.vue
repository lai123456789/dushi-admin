<template>
  <div>
    ...
    <a-spin :spinning="spinning">
      <a-card v-if="flowScope==='all'">
        <a-tabs v-model="flowScopeKey">
          <a-tab-pane tab="进行中" key="proceed">
            <user-table-list-proceed
              ref="UserTableListProceed"
              :params="params"
              :flowStatus="flowScopeKey"
              :viewThis="viewThis"
              :filtersData="filtersData"
              :formThis="formThis"
              :screenData="screenData"
              @ok="(val, type, data)=>$emit('ok', val, type, data)"
            />
          </a-tab-pane>
      ...
      <div v-else>
        <a-card class="table-search" v-show="cardTemplate.length === 0 && loading && (flowScope === 'finish' || template.length > 0)">
          <a-form :form="formSearch" :class="advancedSearch ? 'advanced' : 'normal'">
            <div class="head">
              ...
              <a-space style="margin-left: 8px">
                ...
                <a-button @click="() => {queryParam = {}, sorter.end_date = moment().startOf('month').format('YYYY-MM'), endDate = moment().startOf('month'), searchString = {}, clearAddress(), formSearch.resetFields() ;handleSearch({tag: false})}">重置</a-button>
            ...
            <a-row :gutter="16">
              ...
              <a-col :span="6" v-if="flowScope === 'finish' && timeRangeDisplay !== '0'">
        ...
        <a-card v-show="cardTemplate.length === 0 && loading">
          ...
          <div class="table-operator">
            ...
            <a-row type="flex" align="middle">
              <a-col :span="23">
                <component v-for="(item, index) in tbar" :key="index" :is="item.component" v-show="flowScope === 'finish' && ['sys_add'].includes(item.bar_alias) ? false : true"/>
              ...
              <a-col :span="1" style="text-align: right">
                <a-space>
                  <a-button icon="setting" style="border: none; font-size: 18px; margin-right: 0px" @click="customShow" v-if="customColumns == '1' ? true : false"></a-button>
                </a-space>
              </a-col>
            </a-row>
          </div>
...
export default {
  components: {
    ...
    UserTableListProceed: () => import('./UserTableListProceed')
  },
  props: {
    params: {
      type: Object,
  ...
  created () {
    ...
    this.axios({
      ...
      this.flowScope = res.result.flowScope || 'proceed'
      ...
      this.columns.map((item) => {
        ...
        if (item.dataIndex === 'action') {
          ...
          if (this.params.type === 'table_subform_list' && this.params.selectAction) {
            ...
            obj.attribute = `<a href="javascript:;" @click="() => {  parent.axios({url: '/admin/UserTable/getTableRecord',params: { tableid: parent.tableid, id: record.id, flowStatus: parent.flowScope }}).then(res => { parent.$emit('ok', res.result.data, 'associated', parent.screenData)})}">选择</a>`
            item.rowAction.push(obj)
          }
  ...
  methods: {
    ...
    loadDataTable (parameter) {
      if (!this.params.end_date && this.flowScope === 'finish' && this.timeRangeDisplay !== '0') {
        this.sorter.end_date = ''
        this.sorter.end_date = this.moment().startOf('month').format('YYYY-MM')
      }
      if (this.params.type !== 'table_card_list' && this.flowScope !== 'all') {
        ...
        return this.axios({
          ...
          params: Object.assign(this.sorter, parameter, { flowStatus: this.flowScope }),
      ...
      this.columns.map((item) => {
        ...
        if (item.dataIndex === 'action') {
          ...
          if (this.params.type === 'table_subform_list') {
            ...
            obj.attribute = `<a href="javascript:;" @click="() => {  parent.axios({url: '/admin/UserTable/getTableRecord',params: { tableid: parent.tableid, id: record.id, flowStatus: parent.flowScope }}).then(res => { parent.$emit('ok', res.result.data, 'associated')})}">选择</a>`
            item.rowAction.push(obj)
          }
          if (this.flowScope === 'finish') {
            item.rowAction = item.rowAction.filter(item => item.bar_alias !== 'sys_delete')
    ...
    tableShow (record) {
      record.jointable = this.params.jointable
      record.flowStatus = this.flowScope
      record.tableType = 'table'
      this.indexKey = this.indexKey ? 0 : 1
      this.$nextTick(() => {
        this.$refs.userTableForm.show(record)
      })
    },
    // 高级搜索改变列表值
    ...
    onShow (parameter) {
      ...
      parameter.flowStatus = this.flowScope
      this.indexKey = this.indexKey ? 0 : 1
      this.$nextTick(() => {
