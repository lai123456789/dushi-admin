  ...
  <div>
    <a-card class="table-search" :bordered="false">
      <a-form :layout="advanced ? 'vertical' : 'inline'" :class="advanced ? 'advanced' : 'normal'">
        <a-space>
          <a-button icon="search" type="primary" @click="seatSearch" @keyup.enter="seatSearch" >搜索</a-button>
          ...
          <a-col :span="6">
            <a-form-item label="部门">
              <a-select :allowClear="true" show-search v-model.trim="queryParam.department" >
                <a-select-option v-for="(item, key) in department" :key="key" :value="item.value">{{ item.display }}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :span="6">
