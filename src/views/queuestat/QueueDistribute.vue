  ...
  <div>
    <a-tabs v-model="tbKey" size="small" type="card">
      ...
      <a-tab-pane v-for="item in config.tab" :key="item.key" :tab="item.name">
        ...
        <template v-if="item.key !== 'detailCount'">
          <a-table
            rowKey="key"
            :columns="item.head"
  ...
  methods: {
    init () {
      if (this.changeFlag) {
        ...
        this.axios({
          url: '/queuestat/index/distribution',
          data: {
            type: 'income_detail',
          ...
          if (res.code === 0) {
            ...
            for (const key in keyObj) {
              ...
              if (keyObj[key]) {
                ...
                obj.name = [keyObj[key]]
                if (key === 'division') {
                  obj.value = data.abandon + '/' + data.time_out
                } else {
                  obj.value = data[key]
                }
                arr.push(obj)
    ...
    handleExport (record, key) {
      ...
      if (key) {
        this.$refs.generalExport.show({
          ...
          record: record,
          number: '',
          method: key === 'detailCount' ? 'getalldataExport' : 'distributionExport',
    ...
    loadData (parameter) {
      return this.axios({
        ...
        url: '/queuestat/index/getalldata'
      }).then((res) => {
        return res.result
      })
    },
